...
final class KeyFilters {
  ...
  // number of data files. The budget is what the earlier fixed cap of 100 value ranges (200
  // comparisons per file) spends on a table with one million data files, so such a table plans
  // exactly as before and smaller tables get a finer filter. The lower bound is the earlier
  // behavior and applies when the count is missing; the upper bound limits what the filter costs
  // when every file is read anyway, e.g. keys spread over a small table, where a finer filter only
